...
// Define Collections Globally (Lazy initialization)
...
app.post('/api/v1/category', async (req, res) => {
    try {
        ...
        if (!categoryName) return res.status(400).send({ success: false, message: 'categoryName is required' });
...
app.post('/api/v1/products', async (req, res) => {
    try {
        ...
        if (!productName || !description || price == null || stock == null || !categoryId) return res.status(400).send({ success: false, message: 'Missing required fields' });
        const now = new Date().toISOString();
...
app.put('/api/v1/products/:id', async (req, res) => {
    try {
        ...
        const result = await ProductsCollection.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: updateFields }, { returnDocument: 'after' });
...
app.delete('/api/v1/products/:id', async (req, res) => {
    try {
        const id = req.params.id;
        const result = await ProductsCollection.updateOne({ _id: new ObjectId(id) }, { $set: { isDeleted: true, updatedAt: new Date().toISOString() } });
